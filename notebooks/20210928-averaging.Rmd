...
```{r avg-graph, fig.width=4, fig.height=3}
avg_df <- past_df %>% 
...
```{r fixed-average, fig.width=4, fig.height=3}
avg_df <- past_df %>% 
  mutate(day = yday(date)) %>% 
  filter(hour == 10 | hour == 13 | hour == 16) %>% # Remove the old times
  group_by(day, hour) %>% 
...
```{r final-graph, echo=FALSE, fig.width=4, fig.height=3}
final_df %>% 
  filter(hour == 13) %>% 
  drop_na() %>% 
...
```{r bonus, fig.width=4, fig.height=3}
past_df %>% 
  filter(hour == 13) %>% 
  mutate(
    year = year(date),
