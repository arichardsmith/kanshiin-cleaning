...
There's two ways we can make a plot comparing the current year to average years. The first way is to make an average dataframe with the old data, then combine it with the current years data. The second is quick and easy if your data is all in one dataframe.
...
While it's not statistically correct. For the sake of making a quick graph, the easiest way to deal with this is to tell it to ignore `NA` values with `mean(x, na.rm = TRUE)`
...
ggplot(avg_df, aes(day, temp)) +
  geom_line(aes(color = factor(hour)))
```
At this point you could upload the average data to googlesheets, or save it as a csv.
